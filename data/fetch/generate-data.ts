...
import { slug as slugUtils, wargear as wargearUtils } from './vendor';
import { sortByName } from './vendor';
import type { wahapedia, depot } from './vendor';
import { buildSourceClassifier } from './utils/source-classification';
import type { SourceClassifier } from './utils/source-classification';
import { getSupplementInfo } from './config/supplements';

const CODEX_SLUG = 'codex';

/**
 * Datasheets skipped because their source_id doesn't resolve. Reported at the end
 * of a run so an upstream placeholder never disappears silently.
 */
const skippedForMissingSource: string[] = [];
...
const buildDatasheet = (
  ...
  if (!source) {
    // Wahapedia stages unreleased and placeholder rows with an empty source_id
    // (as of 2026-06: "Example Wargear", plus a handful of unreleased units).
    // A datasheet with no rulebook can't be attributed, so it is skipped —
    // aborting the whole run over one staged row makes every refresh hostage to
    // upstream's work-in-progress. Skips are reported by the caller.
    skippedForMissingSource.push(`${datasheet.name} (${datasheet.id})`);
    return undefined;
  }
...
const buildFactionData = (
  ...
  const datasheets = data.datasheets
    ...
    .filter((datasheet): datasheet is depot.Datasheet => datasheet !== undefined)
    .slice()
...
interface GenerateDataResult {
  ...
  /** Datasheets dropped because their source_id doesn't resolve — see above. */
  skippedForMissingSource: string[];
}
...
const generateData = (): GenerateDataResult => {
  ...
  return { factions, coreStratagems, dataVersion, skippedForMissingSource };
};
